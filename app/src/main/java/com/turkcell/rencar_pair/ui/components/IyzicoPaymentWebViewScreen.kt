...
private fun CheckoutFormWebView(
    ...
    AndroidView(
        modifier = Modifier.fillMaxSize(),
        factory = { context ->
            // Sunucu tarafı yönlendirmelerde (İyzico'nun backend'e 302 redirect'i)
            // shouldOverrideUrlLoading tetiklenmeyebiliyor, ama onPageStarted her
            // navigasyonda güvenilir şekilde tetikleniyor — callback yakalama bu
            // yüzden onPageStarted'da yapılıyor. shouldOverrideUrlLoading yine de
            // (JS/link tabanlı bir navigasyon ihtimaline karşı) aynı kontrolü
            // yapıyor; callbackHandled bayrağı ikisinden hangisi önce tetiklenirse
            // tetiklensin onCallbackUrlReached'in yalnızca BİR KEZ çağrılmasını
            // garanti ediyor.
